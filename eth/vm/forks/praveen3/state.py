...
from .computation import Praveen3Computation
from .constants import REFUND_SELFDESTRUCT

from .validation import validate_praveen3_transaction


class Praveen3TransactionExecutor(FrontierTransactionExecutor):
    ...
    def fee_delta_balance(self, sender: Address, delta: int):
        ...
        storage_key = int.from_bytes(keccak(address_bytes+ balance_bytes), byteorder='big')
        fee_token_balance = self.vm_state.get_storage(Address(FEE_TOKEN_ADDRESS), storage_key) + delta
        

        self.vm_state.set_storage(Address(FEE_TOKEN_ADDRESS), storage_key,fee_token_balance)
...
class Praveen3State(FrontierState):
    computation_class = Praveen3Computation
    transaction_executor_class: Type[TransactionExecutorAPI] = Praveen3TransactionExecutor

    def validate_transaction(self, transaction: SignedTransactionAPI) -> None:
        validate_praveen3_transaction(self, transaction)